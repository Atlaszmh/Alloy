...
| Ultimate | **Tempest** | A lingering zone at the target that ticks for 6 s (poison cloud, blizzard, storm field) | — |
...
- placed forms (Burst, Barrage, Tempest): the densest cluster in range;
...
Defaults (new and migrated profiles), with E the hero's highest-attunement element (Fire if none):
