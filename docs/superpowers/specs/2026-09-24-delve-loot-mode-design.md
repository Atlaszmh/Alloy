...
- **Two sources** (`art/alloy/manifest.json`): `code` sprites are ASCII grids with a
  ...
  either through the API (`generate`, paid from the AI Pro Cloud credit) or by hand in
  the Gemini app (`prompts` writes the prompts and reference sheet, `import` brings the
  saved images back from `art/alloy/inbox/`).
