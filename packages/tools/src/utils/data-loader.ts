...
function generateBaseAffixDescription(id: string, name: string, category: string): string {
  const descriptions: Record<string, string> = {
    ...
    fire_damage: `🔥 **WEAPON**: DoT damage that scales over time—the longer combat continues, the more fire your target is burning in. Persistence is your mechanic. Enemies that stay in combat literally burn harder.\n\n🔥 **ARMOR**: Fire resistance that converts heat into shield. Enemy fire doesn't just bounce off—it fuels defensive barriers. Getting burned makes you tougher.`,

    cold_damage: `❄️ **WEAPON**: Chilling damage that *debilitates* movement and action speed. You're not just dealing damage—you're slowing their response. Each blow makes them sluggish.\n\n❄️ **ARMOR**: Your defenses actively *leech* heat from attacks. Enemy fire becomes weaker around you. You don't just resist cold—you're a heat sink.`,

    lightning_damage: `⚡ **WEAPON**: Damage that *bounces* to nearby enemies. Single-target becomes multi-target automatically. Enemies clustering together becomes their mistake.\n\n⚡ **ARMOR**: Grounds electrical damage harmlessly. Lightning trying to zap you gets redirected into the earth. You're an antenna that prevents strikes.`,
    ...
    armor_rating: `🛡️ **WEAPON**: Hardens your attacks with defensive magics, making even glancing blows count. You punch with reinforced knuckles and swing with weighted force.\n\n🏰 **ARMOR**: Pure, unyielding protection. Your armor is magically hardened—attacks simply bounce off. This is protection distilled to its simplest form: the enemy cannot penetrate you.`,
    ...
    chance_on_crit: `💥 **WEAPON**: Critical strikes unlock special conditions beyond damage. When you *land* the important hits, something extra happens.\n\n⚡ **ARMOR**: When enemies crit YOU, it backfires on them spectacularly. Their risky aggressive plays become their downfall. Reward them for trying to hurt you hard.`,
...
function generateCompoundDescription(id: string, name: string, componentIds: string[], affixes: any[]): string {
  ...
  const descriptions: Record<string, string> = {
    ...
    static_discharge: `"Let there be lightning. Lots of it." Braids ${componentNames} into electrical cascade.\n\n⚡ **WEAPON - BOUNCING CHAIN**: Your strikes don't just damage one enemy—they arc to nearby foes. Each bounce is independent and damages differently. Enemies close together turn into a chain reaction. The more densely packed they are, the more bounces, the more total damage. It's area control through electricity.\n\n📡 **ARMOR - LIGHTNING ROD**: You become the conduit for all nearby electricity. Enemy lightning attacks are pulled toward you and dispersed harmlessly. You're a walking lightning rod—the more they use electricity, the more you drain. Casters get really mad when their spells fuel your defense.`,
