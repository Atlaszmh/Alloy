...
describe('useMatchGateway', () => {
  it('returns null on initial render, then LocalGateway after effect', () => {
    const { result, unmount } = renderHook(() => useMatchGateway('ai-test-1'));
    // After useEffect fires, gateway should be created
  ...
  it('creates new instance when code changes', async () => {
    ...
    // After re-render with new code, effect fires and creates new gateway
