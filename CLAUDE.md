...
- **Controller** (`features/gamepad/`): an Xbox-style pad (the browser's standard mapping), Hades-style: left stick moves, right stick aims (centred = auto-aim), A dodges, X/B/Y abilities, RT manual attack, LB potion, Menu the dive menu. `gamepad-hub.ts` reads the pad once per frame and routes each press to exactly one owner: the arena while a fight is live (`setArenaLive`), else the menu layer (`use-gamepad-nav.ts`: D-pad/stick focus by position, A presses, B presses `[data-pad-back]`, LB/RB step `[data-pad-tabs]`, Menu presses `[data-pad-menu]`, focus kept inside the last `[data-pad-scope]`). E2E fakes a pad by stubbing `navigator.getGamepads` (`e2e/delve-gamepad.spec.ts`).
