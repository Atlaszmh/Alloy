...
- **Mana-pixel FX** (`features/delve/arena/fx/`): every combat effect (shields, zones, telegraphs, projectiles, swings, casts, aim markers, monster marks) is drawn as mana pixels on the sprites' 0.1-unit grid rather than as smooth shapes. `mana-pixels.ts` is the vocabulary (`manaRing`, `manaDust`, `manaMotes`, `manaArc`, `manaLine`, `manaOrb`), `pixel-layer.ts` rasterises a Graphics into a grid-snapped texture (a ground layer and an additive air layer), `draw-world.ts` draws the world's persistent effects each frame, and `mana-fx.ts` holds the short-lived ones plus the casting polish (a fling of mana on every cast and basic, and melee smears that sweep across their arc). Hits carry heft: heavy ones freeze the display briefly (`fx/hitstop.ts`) and kick the camera; wind-ups lean the hero and gather mana (`fx/anticipation.ts`); shots spark at the hand and effects dissolve when they end (`fx/lifecycles.ts`). Cosmetic, so it may use `Math.random`.
